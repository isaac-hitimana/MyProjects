...
class Course:
    ...
    def addStudentsFromFile(self, filename):
        ...
        #  This function add course information from file
    def addCourseDataFromFile(self, filename):
        """
        #Adds courses from file
        Parameters: filename, a file path
        """
        filereader=open(filename)
        lines=filereader.readlines()
        tas=lines[4].strip('\n').split(',')  
        self.setCourseID(lines[0].strip('\n'))
        self.setCourseName(lines[1].strip('\n'))
        self.setSemester(lines[2].strip('\n'))
        self.setInstructor(lines[3].strip('\n'))
        # self.setCourseTAs(tas)
        for ta in tas:
            self.addTA (ta)
        filereader.close()
